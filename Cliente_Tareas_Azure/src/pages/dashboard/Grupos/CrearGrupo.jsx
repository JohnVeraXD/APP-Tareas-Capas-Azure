...
export default function CrearGrupo({ abrir, cerrar, crear }) {

    const [open, setOpen] = useState(true);
    const handleOpen = () => setOpen((cur) => !cur);
    const cookies = new Cookies();

    //estado para almacenar la info de la grupo
    ...
    //funcion para crear la grupo
    const Crear_Grupo = async () => {

        try {
            const result = await axios.post(
                process.env.NEXT_PUBLIC_ACCESLINK + "grupos/CrearGrupo",
                grupo,
                {
            ...
            if (error.response.data.error = "grupos_titulo_key") {
